...
### 3. Simulating Telemetry & Safety Alerts (Carsky Platform)

Because the Carsky cloud platform runs continuous background simulations that hold virtual speed at 0km/h, standard speed injections often fail.
To test the Driver Distraction UI locks on **Carsky**, use the following ADB commands to force the driving state instead:

* **Simulate Drive (D) - Locks the UI**:

  ```powershell
  & $env:LOCALAPPDATA\Android\Sdk\platform-tools\adb.exe -s localhost:5555 shell cmd car_service emulate-driving-state drive
  ```

  *(Or run `cmd car_service emulate-driving-state drive` directly inside the Carsky ADB Widget).*
* **Simulate Park (P) - Unlocks the UI**:

  ```powershell
  & $env:LOCALAPPDATA\Android\Sdk\platform-tools\adb.exe -s localhost:5555 shell cmd car_service emulate-driving-state park
  ```
* **Inject Speed Update (e.g. 50 km/h)**:

  ```powershell
  & $env:LOCALAPPDATA\Android\Sdk\platform-tools\adb.exe -s localhost:5555 shell cmd car_service inject-vhal-event 0x11600207 50
  ```

  *(Note: Speed injection may be overridden by active signal nodes on Carsky. Use `emulate-driving-state` for guaranteed UI lock testing).*
